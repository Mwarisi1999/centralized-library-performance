...
@section('content')
    ...
    @if(auth()->user()->hasRole('Administrator'))
        <section class="mt-6 space-y-4"><div class="grid grid-cols-2 gap-4 md:grid-cols-5">@foreach([['Users',$adminSummary['users']],['Inactive Accounts',$adminSummary['inactive_accounts']],['Active Campuses',$adminSummary['active_campuses']],['Pending Task Reviews',$adminSummary['pending_task_reviews']],['Pending Reports',$adminSummary['pending_reports']]] as [$label,$value])<article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p class="text-xs font-semibold uppercase text-slate-500">{{ $label }}</p><p class="mt-2 text-2xl font-bold">{{ $value }}</p></article>@endforeach</div><div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="text-lg font-bold">Administrator quick actions</h2><p class="mt-1 text-sm text-slate-500">Manage access, organization data, and application governance.</p></div><div class="flex flex-wrap gap-2"><a href="{{ route('admin.staff.index') }}" class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">Staff Management</a><a href="{{ route('admin.organization.index') }}" class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">Organization Setup</a><a href="{{ route('admin.administration.index') }}" class="rounded-xl bg-emerald-800 px-4 py-2 text-sm font-semibold text-white">Administration</a></div></div></div></section>
    @endif
